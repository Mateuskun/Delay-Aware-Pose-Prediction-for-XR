...
img_xfer_cb(struct libusb_transfer *xfer)
{
	...
	const int64_t host_transfer_complete_ns = (int64_t)os_monotonic_get_ns();
	...
	if (slam_tracking_frame) {
		...
		const int64_t host_driver_pushed_ns = (int64_t)os_monotonic_get_ns();
		for (int i = 0; i < cam->slam_cam_count; i++) {
			frames[i]->host_transfer_complete_ns = host_transfer_complete_ns;
			frames[i]->host_driver_pushed_ns = host_driver_pushed_ns;
