...
euroc_player_load_next_frame(struct euroc_player *ep, int cam_index, struct xrt_frame *&xf)
{
	...
	timepoint_ns host_exposure = euroc_player_mapped_ts(ep, sample.first);
	...
	xf->host_exposure_ns = host_exposure;
	xf->host_transfer_complete_ns = os_monotonic_get_ts();
}
...
euroc_player_push_next_frame(struct euroc_player *ep)
{
	...
	timepoint_ns host_driver_pushed_ns = os_monotonic_get_ts();
	for (int i = 0; i < cam_count; i++) {
		xfs[i]->host_driver_pushed_ns = host_driver_pushed_ns;
...
#define DEFINE_RECEIVE_CAM(cam_id)                                                                                     \
	...
	{                                                                                                              \
		xf->host_source_received_ns = os_monotonic_get_ts();                                                   \
		...
		if (ep->out_sinks.cams[cam_id]) {                                                                      \
			xf->host_source_pushed_ns = os_monotonic_get_ts();                                             \
