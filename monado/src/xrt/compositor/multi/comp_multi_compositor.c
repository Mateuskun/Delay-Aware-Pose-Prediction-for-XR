                               ...
                               int64_t *out_predicted_display_period_ns)
{
	...
	*out_predicted_gpu_time_ns = 0;
	u_timing_pipeline_mark_wait_frame(*out_frame_id, *out_predicted_display_time_ns, now_ns);
...
multi_compositor_begin_frame(struct xrt_compositor *xc, int64_t frame_id)
{
	...
	os_mutex_unlock(&mc->msc->list_and_timing_lock);
	u_timing_pipeline_mark_begin_frame(frame_id, now_ns);
...
multi_compositor_layer_commit(struct xrt_compositor *xc, xrt_graphics_sync_handle_t sync_handle)
{
	...
	int64_t host_present_ns = os_monotonic_get_ns();
	u_timing_pipeline_mark_present_frame(frame_id, host_present_ns);
