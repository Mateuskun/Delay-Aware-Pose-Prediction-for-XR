...
// the app pacer does different things depending on the state of the frame
// We need to track different things depending on the event

enum u_pa_csv_event
{
	U_PA_CSV_EVENT_INFO = 0,
	U_PA_CSV_EVENT_PREDICT = 1,
	U_PA_CSV_EVENT_GPU_DONE = 2,
};

enum u_pa_csv_column
{
	U_PA_CSV_EVENT,
	U_PA_CSV_FRAME_ID,
	U_PA_CSV_NOW_NS,
	U_PA_CSV_IN_DISPLAY_TIME,
	U_PA_CSV_IN_PERIOD,
	U_PA_CSV_IN_EXTRA,
	U_PA_CSV_RAW_WAIT_WOKE,
	U_PA_CSV_RAW_BEGIN,
	U_PA_CSV_RAW_DELIVERED,
	U_PA_CSV_RAW_GPU_DONE,
	U_PA_CSV_OUT_WAKE,
	U_PA_CSV_OUT_DISPLAY_TIME,
	U_PA_CSV_OUT_APP_PERIOD,
	U_PA_CSV_APP_CPU,
	U_PA_CSV_APP_DRAW,
	U_PA_CSV_APP_GPU,
	U_PA_CSV_COL_COUNT,
...
static const char *const u_pa_csv_columns[U_PA_CSV_COL_COUNT] = {
    "event",        "frame_id",    "now_ns",         "in_display_time",
    "in_period",    "in_extra",    "raw_wait_woke",  "raw_begin",
    "raw_delivered", "raw_gpu_done", "out_wake",      "out_display_time",
    "out_app_period", "app_cpu",   "app_draw",       "app_gpu",
};
...
struct pacing_app
{
	...
	struct u_frame_timing_writer *csv_writer;
};
...
static inline void
pa_csv_clear(int64_t row[U_PA_CSV_COL_COUNT])
{
	for (size_t i = 0; i < U_PA_CSV_COL_COUNT; i++) {
		row[i] = U_FTW_UNSET;
	}
}

static void
pa_csv_push(struct pacing_app *pa, const int64_t row[U_PA_CSV_COL_COUNT])
{
	if (pa->csv_writer != NULL) {
		u_frame_timing_writer_push(pa->csv_writer, row);
	}
}
           ...
           int64_t *out_predicted_display_period)
{
	...
	*out_predicted_display_period = app_period_ns;
	...
	if (pa->csv_writer != NULL) {
		int64_t row[U_PA_CSV_COL_COUNT];
		pa_csv_clear(row);
		row[U_PA_CSV_EVENT] = U_PA_CSV_EVENT_PREDICT;
		row[U_PA_CSV_FRAME_ID] = frame_id;
		row[U_PA_CSV_NOW_NS] = now_ns;
		row[U_PA_CSV_IN_DISPLAY_TIME] = pa->last_input.predicted_display_time_ns;
		row[U_PA_CSV_IN_PERIOD] = pa->last_input.predicted_display_period_ns;
		row[U_PA_CSV_IN_EXTRA] = pa->last_input.extra_ns;
		row[U_PA_CSV_OUT_WAKE] = wake_up_time_ns;
		row[U_PA_CSV_OUT_DISPLAY_TIME] = predict_ns;
		row[U_PA_CSV_OUT_APP_PERIOD] = app_period_ns;
		row[U_PA_CSV_APP_CPU] = pa->app.cpu_time_ns;
		row[U_PA_CSV_APP_DRAW] = pa->app.draw_time_ns;
		row[U_PA_CSV_APP_GPU] = pa->app.gpu_time_ns;
		pa_csv_push(pa, row);
	}
...
pa_mark_gpu_done(struct u_pacing_app *upa, int64_t frame_id, int64_t when_ns)
{
	...
	if (pa->csv_writer != NULL) {
		int64_t row[U_PA_CSV_COL_COUNT];
		pa_csv_clear(row);
		row[U_PA_CSV_EVENT] = U_PA_CSV_EVENT_GPU_DONE;
		row[U_PA_CSV_FRAME_ID] = frame_id;
		row[U_PA_CSV_RAW_WAIT_WOKE] = f->when.wait_woke_ns;
		row[U_PA_CSV_RAW_BEGIN] = f->when.begin_ns;
		row[U_PA_CSV_RAW_DELIVERED] = f->when.delivered_ns;
		row[U_PA_CSV_RAW_GPU_DONE] = f->when.gpu_done_ns;
		row[U_PA_CSV_APP_CPU] = pa->app.cpu_time_ns;
		row[U_PA_CSV_APP_DRAW] = pa->app.draw_time_ns;
		row[U_PA_CSV_APP_GPU] = pa->app.gpu_time_ns;
		pa_csv_push(pa, row);
        ...
        int64_t extra_ns)
{
	...
	if (pa->csv_writer != NULL) {
		int64_t row[U_PA_CSV_COL_COUNT];
		pa_csv_clear(row);
		row[U_PA_CSV_EVENT] = U_PA_CSV_EVENT_INFO;
		row[U_PA_CSV_IN_DISPLAY_TIME] = predicted_display_time_ns;
		row[U_PA_CSV_IN_PERIOD] = predicted_display_period_ns;
		row[U_PA_CSV_IN_EXTRA] = extra_ns;
		pa_csv_push(pa, row);
	}
}
...
pa_destroy(struct u_pacing_app *upa)
{
	...
	if (pa->csv_writer != NULL) {
		u_frame_timing_writer_destroy(&pa->csv_writer);
	}
...
pa_create(int64_t session_id, struct u_pacing_app **out_upa)
{
	...
	const char *csv_path = getenv("MONADO_PACER_CSV");
	if (csv_path != NULL && session_id == 0) {
		pa->csv_writer = u_frame_timing_writer_create(csv_path, u_pa_csv_columns, U_PA_CSV_COL_COUNT);
		if (pa->csv_writer == NULL) {
			UPA_LOG_E("Could not open MONADO_PACER_CSV '%s'", csv_path);
		}
	}
