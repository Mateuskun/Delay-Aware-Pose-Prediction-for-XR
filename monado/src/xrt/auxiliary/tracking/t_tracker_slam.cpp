...
public:
	...
	void
	set_column_names(const vector<string> &cn)
	{
		if (created) {
			U_LOG_W("Setting column names after creation won't have any effect");
		}
		column_names = cn;
	}
};
...
ostream &
operator<<(ostream &os, const imu_sample &s)
{
	os << s.ts << ",";
	os << s.ax << "," << s.ay << "," << s.az << ",";
	os << s.wx << "," << s.wy << "," << s.wz << CSV_EOL;
	return os;
}

struct ImuWriter : public CSVWriter<imu_sample>
{
	ImuWriter(const string &dir, const string &fn, bool e) : CSVWriter<imu_sample>(dir, fn, e)
	{
		column_names = {"timestamp [ns]", "a_RS_S_x [m s^-2]", "a_RS_S_y [m s^-2]", "a_RS_S_z [m s^-2]",
		                "w_RS_S_x [rad s^-1]", "w_RS_S_y [rad s^-1]", "w_RS_S_z [rad s^-1]"};
	}
};

struct relation_sample
{
	timepoint_ns ts;
...
ostream &
operator<<(ostream &os, const relation_sample &s)
{
	const xrt_vec3 &p = s.rel.pose.position;
	const xrt_quat &r = s.rel.pose.orientation;
	const xrt_vec3 &lv = s.rel.linear_velocity;
	const xrt_vec3 &av = s.rel.angular_velocity;
	os << s.ts << ",";
	os << p.x << "," << p.y << "," << p.z << ",";
	os << r.w << "," << r.x << "," << r.y << "," << r.z << ",";
	os << lv.x << "," << lv.y << "," << lv.z << ",";
	os << av.x << "," << av.y << "," << av.z << CSV_EOL;
	return os;
}

struct RelationWriter : public CSVWriter<relation_sample>
{
	RelationWriter(const string &dir, const string &fn, bool e) : CSVWriter<relation_sample>(dir, fn, e)
	{
		column_names = {"timestamp [ns]", "p_RS_R_x [m]", "p_RS_R_y [m]", "p_RS_R_z [m]",
		                "q_RS_w []",      "q_RS_x []",    "q_RS_y []",    "q_RS_z []",
...
struct TrackerSlam
{
	...
	RelationWriter *slam_rel_writer;

	map<timepoint_ns, vector<camera_timing>> camera_timing_by_ts;
...
timing_ui_setup(TrackerSlam &t)
{
	...
	t.slam_times_writer->set_column_names(t.timing.columns);

	if (t.slam_times_writer->enabled && t.exts.has_pose_timing) {
		vit_result_t vres = t.vit.tracker_enable_extension(t.tracker, VIT_TRACKER_EXTENSION_POSE_TIMING, true);
		if (vres == VIT_SUCCESS) {
			t.timing.enabled = true;
			SLAM_INFO("Auto-enabled VIT pose timing extension for CSV writing");
		} else {
			SLAM_WARN("Failed to auto-enable VIT pose timing extension");
		}
	}
...
static void
create_camera_timing(TrackerSlam &t, timepoint_ns pose_ts, int64_t host_flushed_ns)
{
	auto it = t.camera_timing_by_ts.find(pose_ts);
	if (it == t.camera_timing_by_ts.end()) {
		return;
	}

	for (const camera_timing &sample : it->second) {
		u_timing_pipeline_push_camera(sample.exposure_ns, sample.host_usb_transfer_done_ns,
		    sample.host_sent_to_basalt_ns,
		    host_flushed_ns,
		    sample.cam_index,
		    sample.source_sequence);
	}

	t.camera_timing_by_ts.erase(it);
}
...
flush_poses(TrackerSlam &t)
{
	...
	do {
		...
		// Timestamp right after sending it to basalt
		const int64_t host_pose_flushed_ns = (int64_t)os_monotonic_get_ns();
		create_camera_timing(t, nts, host_pose_flushed_ns);
		...
		if (t.dbg_pred_counter % t.dbg_pred_every == 0) {
			...
			t.slam_rel_writer->push({nts, rel});
		}
...
setup_ui(TrackerSlam &t)
{
	...
	u_var_add_bool(&t, &t.imu_writer->enabled, "Record IMU samples");
	u_var_add_bool(&t, &t.slam_rel_writer->enabled, "Record SLAM relations (with velocities)");
...
t_slam_get_tracked_pose(struct xrt_tracked_slam *xts, timepoint_ns when_ns, struct xrt_space_relation *out_relation)
{
	...
	const int64_t host_request_ns = (int64_t)os_monotonic_get_ns();
	u_timing_pipeline_mark_locate_views(when_ns, host_request_ns);
	...
	const int64_t host_predict_done_ns = (int64_t)os_monotonic_get_ns();
	u_timing_pipeline_mark_predict_filter(when_ns, host_predict_done_ns);
...
t_slam_receive_imu(struct xrt_imu_sink *sink, struct xrt_imu_sample *s)
{
	...
	t.imu_writer->push({ts, a.x, a.y, a.z, w.x, w.y, w.z});
}
...
receive_frame(TrackerSlam &t, struct xrt_frame *frame, uint32_t cam_index)
{
	...
	int64_t host_slam_pushed_vit_ns = 0;
	{
		XRT_TRACE_IDENT(slam_push);
		host_slam_pushed_vit_ns = (int64_t)os_monotonic_get_ns();
		t.vit.tracker_push_img_sample(t.tracker, &sample);
	}

	camera_timing camera_timing = {
	    (int64_t)frame->timestamp,
	    frame->host_transfer_complete_ns != 0 ? frame->host_transfer_complete_ns : U_FTW_UNSET,
	    host_slam_pushed_vit_ns,
	    (int64_t)cam_index,
	    (int64_t)frame->source_sequence,
	};
	t.camera_timing_by_ts[(timepoint_ns)frame->timestamp].push_back(camera_timing);
}
              ...
              struct xrt_slam_sinks **out_sink)
{
	...
	t.imu_writer = new ImuWriter(dir, "imu.csv", write_csvs);
	t.slam_rel_writer = new RelationWriter(dir, "slam_relations.csv", write_csvs);
